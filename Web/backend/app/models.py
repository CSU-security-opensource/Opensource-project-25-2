...
class Plant(Base):
    ...
    # ★ WeatherForecast 관계 추가 (필수)
    weather_forecasts = relationship("WeatherForecast", back_populates="plant")
...
class Weather(Base):
    ...
    insolation = Column(Float, nullable=True)     # ☀ 일사량
    humidity = Column(Float, nullable=True)
    cloud_cover = Column(Float, nullable=True)    # ☁ 구름량

    __table_args__ = (
        UniqueConstraint('plant_id', 'timestamp', name='_weather_plant_timestamp_uc'),
    )
...
class WeatherForecast(Base):
    ...
    # ★ 관계 완전 수정 — 이게 정답!
    plant = relationship("Plant", back_populates="weather_forecasts")
